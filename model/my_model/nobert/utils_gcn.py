...
def build_dataset(config, ues_word):
    ...
    def load_dataset(path, graph_path,context_pad_size = 128):
        contents = []
        ...
        with open(path, 'r', encoding='UTF-8') as f:
            ...
            for line in reader:
                ...
                count += 1
                contents.append((context_line, int(label), context_seq_len, graph_line))
        return contents  # [([...], 0), ([...], 1), ...]
    train = load_dataset(config.train_path, config.train_graph_path, config.context_pad_size)
    dev = load_dataset(config.dev_path, config.dev_graph_path, config.context_pad_size)
...
class DatasetIterater(object):
    ...
    def _to_tensor(self, datas):
        if len(datas[0]) == 4:
            ...
            return (context, context_seq_len, context_graph), label
