...
def build_dataset(config, ues_word):
    ...
    def load_dataset(path, graph_path,context_pad_size = 300, target_pad_size = 128):
        contents = []
        graphs = read_pickle(graph_path)
        count = 0
        with open(path, 'r', encoding='UTF-8') as f:
            ...
            for line in reader:
                ...
                graph_line = graphs[count]
                count += 1
                contents.append((context_line, int(label), context_seq_len, graph_line, target_line, target_seq_len))
        return contents  # [([...], 0), ([...], 1), ...]
    train = load_dataset(config.train_path, config.train_graph_path, config.context_pad_size, config.target_pad_size)
    dev = load_dataset(config.dev_path, config.dev_graph_path, config.context_pad_size, config.target_pad_size)
...
class DatasetIterater(object):
    ...
    def _to_tensor(self, datas):
        if len(datas[0]) == 6:
            ...
            context_graph = torch.LongTensor([_[3] for _ in datas]).to(self.device)
            target = torch.LongTensor([_[4] for _ in datas]).to(self.device)
            target_seq_len = torch.LongTensor([_[5] for _ in datas]).to(self.device)

            return (context, context_seq_len, context_graph, target, target_seq_len), label
